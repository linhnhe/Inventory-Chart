InventoryDaysToExpirySM1 <-
  function(data, type){
    ...
    #   type: a list of types (classification) of inventory to be plotted
    #
    # Returns:
    ...
    sku.list <- data %>% group_by(SKU) %>% summarise(Max.Inventory = max(Quantity), Inventory.Type = first(Type))
    sku.list <- sku.list[order(sku.list$Max.Inventory, decreasing = TRUE),]
    sku.list <- filter(sku.list, Inventory.Type %in% type)
